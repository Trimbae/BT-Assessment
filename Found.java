...
class Found implements Notification{
	...
	//constructor
	public Found(String notificationTimeReceived, String notificationTimeSent, Node notificationSentFrom, Node notificationSubjectNode){
	...
	public boolean checkForUnknown(Node currentNode){
		...
		if(Math.abs(timeSent - currentNode.getTimeLastSent()) < 50){
			...
			currentNode.setLastNotificationUnknown(nodeSentFrom.getName() + " FOUND " + subjectNode.getName());
	...
	public void doUpdate(){
		if(!checkForUnknown(nodeSentFrom)){
			...
			nodeSentFrom.setLastNotification(nodeSentFrom.getName() + " FOUND " + subjectNode.getName());
		}
		if(!checkForUnknown(subjectNode)){
			subjectNode.setStatus("ALIVE");
			subjectNode.setTimeLastReceived(timeReceived);
			subjectNode.setTimeLastSent(timeSent);
			subjectNode.setLastNotification(nodeSentFrom.getName() + " FOUND " + subjectNode.getName());
		}
	}
